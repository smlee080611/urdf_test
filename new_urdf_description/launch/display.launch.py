...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='new_urdf_description').find('new_urdf_description')
    default_model_path = os.path.join(pkg_share, 'urdf/new_urdf.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/new_urdf/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'new_urdf', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
