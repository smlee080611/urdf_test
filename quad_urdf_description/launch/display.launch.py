...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='quad_urdf_description').find('quad_urdf_description')
    default_model_path = os.path.join(pkg_share, 'urdf/quad_urdf.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/quad_urdf/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'quad_urdf', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
