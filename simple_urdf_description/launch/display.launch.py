...
def generate_launch_description():
    pkg_share = launch_ros.substitutions.FindPackageShare(package='simple_urdf_description').find('simple_urdf_description')
    default_model_path = os.path.join(pkg_share, 'urdf/simple_urdf.xacro')
    default_rviz_config_path = os.path.join(pkg_share, 'config/display.rviz')
    world_path = os.path.join(pkg_share, 'worlds/room.sdf')
    # sdf_path = os.path.join(pkg_share, 'models/urdf/simple_urdf/model.sdf')
    ...
    spawn_entity = launch_ros.actions.Node(
        ...
        arguments=['-entity', 'simple_urdf', '-topic', 'robot_description'],
        parameters=[{'use_sim_time': use_sim_time}],
        output='screen'
    )
